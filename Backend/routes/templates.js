...
// GET all templates - ADD SCHOOL FILTERING
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    // If admin, only show their school's templates
    ...
    const templates = await Template.find(query).sort({ isDefault: -1, createdAt: -1 });
...
// UPLOAD new template - ADD schoolId FROM AUTH
router.post('/upload', authMiddleware, upload.fields([
  ...
  try {
    ...
    // Create template with schoolId from auth
    const template = new Template({
      name,
      description,
      schoolId: req.user.schoolId, // Add schoolId
      frontSide: {
    ...
    // If set as default, unset other defaults for this school
...
// SET default template - ADD SCHOOL CHECK
router.patch('/:id/set-default', authMiddleware, async (req, res) => {
  try {
    ...
    // Check if admin has access to this template
    if (req.user.role === 'admin' && template.schoolId?.toString() !== req.user.schoolId?.toString()) {
      return res.status(403).json({ success: false, error: 'Access denied' });
    }

    // Unset all defaults for this school
    await Template.updateMany(
...
// DELETE Template - ADD SCHOOL CHECK
